...
public class RingBuffersTests
{
    ...
    public void ProcessorMemoryReorderingTest()
    {
        ...
        Thread write1Thread = new Thread(o =>
        {
            ...
            while (noLockRingBuffer.ProcessedCount < TARGET_COUNT * capacity)
            {
                noLockRingBuffer.TryEnqueue(buffer);
                noLockRingBuffer.TryEnqueue(buffer);
                noLockRingBuffer.TryEnqueue(buffer);
        ...
        Thread read1Thread = new Thread(o =>
        {
            ...
            while (noLockRingBuffer.ProcessedCount < TARGET_COUNT * capacity)
            {
                noLockRingBuffer.TryDequeue(buffer);
                noLockRingBuffer.TryDequeue(buffer);
                noLockRingBuffer.TryDequeue(buffer);
                noLockRingBuffer.TryDequeue(buffer);
                noLockRingBuffer.TryDequeue(buffer);
        ...
        ulong processedPerSecond = noLockRingBuffer.ProcessedCount / (Convert.ToUInt64(sw.ElapsedMilliseconds) / 1000UL);
