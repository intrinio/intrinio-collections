...
/// </summary>
public class MemMapDelayDynamicBlockUnsafeRingBuffer: IDynamicBlockRingBuffer, IDisposable
{
    #region Data Members
    ...
    public MemMapDelayDynamicBlockUnsafeRingBuffer(uint delayMilliseconds, uint blockSize, ulong blockCapacity, string fileDirectory, string fileNamePrefix, System.Diagnostics.Stopwatch? stopwatch = default, ulong targetPageSize = DefaultTargetPageSize)
    {
        if (blockSize == 0u)
    ...
    /// <summary>
    /// Not thread-safe try enqueue.  This is not safe for calling concurrently, and intended for use with the same producer as consumer. Detects and stores the length of the input block for use in dequeue.
    ...
    /// <summary>
    /// Thread-unsafe try dequeue.  This is not safe for calling concurrently, and intended for use with the same producer as consumer.  Parameter "fullBlockBuffer" MUST be of length BlockSize or greater!
    /// </summary>
