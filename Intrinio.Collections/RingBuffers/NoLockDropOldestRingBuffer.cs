...
/// </summary>
public class NoLockDropOldestRingBuffer : IRingBuffer
{
    #region Data Members
    ...
    public NoLockDropOldestRingBuffer(uint blockSize, ulong blockCapacity)
    {
        ...
        _slotStates = new int[(int)blockCapacity]; // Initialized to 0 (free)
...
/// </summary>
public class NoLockDropOldestRingBuffer<T> : IRingBuffer<T> where T : struct
{
    #region Data Members
    private readonly T[] _data;
    private readonly int[] _slotStates; // CAS, 0=free, 1=written, 2=consuming
    ...
    /// <summary>
    /// A thread-safe implementation of a circular buffer (multiple producer and multiple consumer).  Full behavior: the oldest <see cref="T"/> in the ring buffer will be dropped. 
    /// </summary>
    /// <param name="capacity">The fixed capacity.</param>
    public NoLockDropOldestRingBuffer(ulong capacity)
    {
        ...
        _slotStates = new int[(int)capacity]; // int[] initialized to 0 (free)
    }
        ...
        while (true)
        {
            ...
            if (currentWrite - cachedRead >= _capacity)
            {
                ...
                if (currentWrite - actualRead >= _capacity)
                {
                    ...
                    _producerCachedRead.Value = Volatile.Read(ref _nextReadIndex);
                    continue;
                }
                _producerCachedRead.Value = actualRead;
            }
