...
/// <summary>
/// A read thread-safe, write thread-safe implementation of the <see cref="IDynamicBlockRingBuffer"/> (multiple producer and multiple consumer), with support for tracking the used size of each byte-block.  Full behavior: the block trying to be enqueued will be dropped. 
/// </summary>
public class DynamicBlockNoLockRingBuffer: IDynamicBlockRingBuffer
{
    #region Data Members
    ...
    /// <summary>
    /// A read thread-safe, write thread-safe implementation of the <see cref="IDynamicBlockRingBuffer"/> (multiple producer and multiple consumer).  Full behavior: the block trying to be enqueued will be dropped. Provides support for dealing with blocks of varying size less than or equal to block size. 
    /// </summary>
    ...
    public DynamicBlockNoLockRingBuffer(uint blockSize, ulong blockCapacity)
    {
        ...
        _blockLengths = new int[(int)blockCapacity];
    ...
    public bool TryEnqueue(ReadOnlySpan<byte> blockToEnqueue)
    {
        ...
        while (true)
        {
            ...
            if (Interlocked.CompareExchange(ref _blockNextWriteIndex, nextWrite, currentWrite) == currentWrite)
            {
                ...
                Volatile.Write(ref _blockLengths[(int)slot], length);
    ...
    public bool TryDequeue(Span<byte> fullBlockBuffer, out Span<byte> trimmedBuffer)
    {
        ...
        while (true)
        {
            ...
            if (Interlocked.CompareExchange(ref _blockNextReadIndex, nextRead, currentRead) == currentRead)
            {
                ...
                int length = Volatile.Read(ref _blockLengths[(int)slot]);
