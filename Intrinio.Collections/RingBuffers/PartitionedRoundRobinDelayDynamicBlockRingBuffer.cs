...
/// <summary>
/// A fixed size group of <see cref="Intrinio.Collections.RingBuffers.DelayDynamicBlockSingleProducerRingBuffer"/> partitioned by an index, so that multiple writers may have their own write channel without being locked, while consumption is channel agnostic, thread-safe, and performed in a round-robin style.
/// </summary>
public class PartitionedRoundRobinDelayDynamicBlockRingBuffer : IPartitionedDynamicBlockRingBuffer
{
    private readonly ulong _concurrency;
    private ulong _readIndex;
    private readonly DelayDynamicBlockSingleProducerRingBuffer[] _queues;
    ...
    public ulong Count
    {
        ...
        {
            ulong sum = 0UL;
            foreach (DelayDynamicBlockSingleProducerRingBuffer queue in _queues)
                sum += queue.Count;
            return sum;
        }
    }
    ...
    /// <summary>
    /// A fixed size group of <see cref="Intrinio.Collections.RingBuffers.DelayDynamicBlockSingleProducerRingBuffer"/> partitioned by an index, so that multiple writers may have their own write channel without being locked, while consumption is channel agnostic, thread-safe, and performed in a round-robin style.  Provides support for dealing with blocks of varying size less than or equal to block size. .
    /// </summary>
    ...
    public PartitionedRoundRobinDelayDynamicBlockRingBuffer(uint concurrency, uint blockSize, ulong eachQueueBlockCapacity, uint delayMilliseconds, System.Diagnostics.Stopwatch? stopwatch = default)
    {
        if (concurrency == 0U)
        ...
        _queues = new DelayDynamicBlockSingleProducerRingBuffer[concurrency];
        for (int i = 0; i < concurrency; i++)
            _queues[i] = new DelayDynamicBlockSingleProducerRingBuffer(delayMilliseconds, blockSize, eachQueueBlockCapacity, stopwatch);
    }
