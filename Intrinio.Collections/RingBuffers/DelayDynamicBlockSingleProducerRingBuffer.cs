...
/// </summary>
public class DelayDynamicBlockSingleProducerRingBuffer: IDynamicBlockRingBuffer
{
    #region Data Members
    ...
    public DelayDynamicBlockSingleProducerRingBuffer(uint delayMilliseconds, uint blockSize, ulong blockCapacity, System.Diagnostics.Stopwatch? stopwatch = default)
    {
...
/// A read thread-safe, write not thread-safe implementation of the <see cref="IDynamicBlockRingBuffer"/> (single producer and multiple consumer), with support for tracking the used size of each byte-block.  Full behavior: the block trying to be enqueued will be dropped. 
/// </summary>
public class DelaySingleProducerRingBuffer<T> : IRingBuffer<T> where T : struct
{
    #region Data Members
    ...
    /// <param name="stopwatch">The stopwatch to use for comparison of elapsed milliseconds at time of dequeue to the elapsed milliseconds at the time a partition block was enqueued.</param>
    public DelaySingleProducerRingBuffer(uint delayMilliseconds, ulong capacity, System.Diagnostics.Stopwatch? stopwatch = default)
    {
        _capacity = capacity;
        _delayMilliseconds = Convert.ToInt64(delayMilliseconds);
        _processed = 0UL;
