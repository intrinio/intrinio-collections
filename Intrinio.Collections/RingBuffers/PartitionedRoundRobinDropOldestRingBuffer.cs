...
/// <summary>
/// A fixed size group of <see cref="Intrinio.Collections.RingBuffers.DropOldestSingleProducerRingBuffer"/> partitioned by an index, so that multiple writers may have their own write channel without being locked, while consumption is channel agnostic, thread-safe, and performed in a round-robin style.
/// </summary>
public class PartitionedRoundRobinDropOldestRingBuffer : IPartitionedRingBuffer
{
    private readonly ulong _concurrency;
    private ulong _readIndex;
    private readonly DropOldestSingleProducerRingBuffer[] _queues;
    ...
    public ulong Count
    {
        ...
        {
            ulong sum = 0UL;
            foreach (DropOldestSingleProducerRingBuffer queue in _queues)
                sum += queue.Count;
            return sum;
        }
    }
    ...
    public PartitionedRoundRobinDropOldestRingBuffer(uint concurrency, uint blockSize, uint eachQueueBlockCapacity)
    {
        if (concurrency == 0U)
        ...
        _queues = new DropOldestSingleProducerRingBuffer[concurrency];
        for (int i = 0; i < concurrency; i++)
            _queues[i] = new DropOldestSingleProducerRingBuffer(blockSize, eachQueueBlockCapacity);
    }
